...
_active_model = None
_model_type:str | None = None # use gemini or fall back

def _probe_gemini():
    """try one call to verify grmini is reachable, Returns model or None"""
    try :
        ...
        logfire.info("Gemini embeddings are ready(gemini-embedding-2-preview,3072-dim).")
        return model
...
def get_embedding_dim() -> int:
    """Return the vector dimension for the active model. call afetr _init()."""
    _init()
...
def embed_texts(texts:list[str]) ->list[list[float]]:
    ...
    for i in range(0,len(texts),BATCH_SIZE):
        ...
        with logfire.span("Embed batch", model=_model_type, batch_size=len(batch)):
